...
public:
    ...
    unsigned long lastUpdateCheckTimeStamp = 0;
    ...
    void update() {
        ...
        if (currentSongPositionMs < currentSong.durationMs && isPlaying) {
            currentSongPositionMs += (currentTime - lastUpdateCheckTimeStamp);
        }
...
void SpotConn::trackInfoCallback(void* optParm, AsyncHTTPRequest* request, int readyState) {
    if (readyState == readyStateDone) {
        ...
        spotifyConnection.lastUpdateCheckTimeStamp = millis();
    }
}
...
public:
  ...
  void showNets(int numNets, int selected){ // it shows all networks
    ...
    clearScreen();
    LCDdraw(0, 0, "Networks - " + String(numNets)); // menu name and number of networks
    
    // Print first network
    String id1 = WiFi.SSID(showNum);
    LCDdraw(0, 1, String(((selected % 2) == 0)? "- " : "->") + id1);
    
    // Print second network if available
    if (showNum + 1 < numNets) {
       String id2 = WiFi.SSID(showNum + 1);
       LCDdraw(0, 2, String(((selected % 2) != 0)? "- " : "->") + id2);
    } else {
       clearLine(2); // Clear line if no second network
    }
    
    LCDdraw(0, 3, "            < E > S"); // shows action bar
  }

  void netMenu(int selected, int mode){ // individual network menu
    clearScreen();
    LCDdraw(0, 0, WiFi.SSID(selected)); // Net name
    
    int encType = WiFi.encryptionType(selected);
    String enc = printEncryptionType(encType);
    
    LCDdraw(0, 1, "Encryption: " + enc); // encryption type
    ...
    case ENC_TYPE_NONE:
        switch (mode){
        case 0:
            menuLine = "connect"; // connect
            break;
        case 1:
            menuLine = "RSSI" + String(WiFi.RSSI(selected));
            break;
        default:
            break;}
        break;
    ...
    case ENC_TYPE_CCMP: // WPA2
        ...
        case 0:
            ...
            break;
        case 1:
            menuLine = "RSSI" + String(WiFi.RSSI(selected)); // signal strength
            break;
        case 2:
            menuLine = "password"; 
            break;
  ...
  void drawKeyboard(String menuText, String writenText, char leter, int selected, bool writing){
    ...
    LCDdraw(0, 1, writeLine); // writes the actual text
    ...
    cursorLine += selector;
    LCDdraw(0, 2, cursorLine);
    
    LCDdraw(0, 3, String((writing)? "            < S > U" : "            < W > S")); //menu action bar
  }
  ...
  // Helper method for setup status messages
  void showSetupStatus(String message) {
    LCDdraw(0, 3, message + String("                    ").substring(0, 20 - message.length())); // Pad with spaces
  }
...
void setup(){
  ...
  // Initialize LCD manager with screen dimensions (20x4)
  LCDm.initScreen(20, 4);
  ...
  timeClient.begin(); 
  updateRTCTime(); 
  LCDm.showSetupStatus("connected to NTP");
  ...
  if (serverOn) {
      ...
      LCDm.showSetupStatus("HTTPS server Started");
  }
  ...
  if (!serverOn) {
      ...
      LCDm.showSetupStatus("Start sequence DONE");
      delay(500);
  }
